...
test_array[] = {
    ...
    { .dictionary = NULL, .word =  NULL, .expected_result = 0 },
};
...
int main() {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(test_array); i++) {
        ...
        int lookup_result = invoke_and_test(&received_result, (void(*)())dictionary_lookup, 2, test_dictionary, test_array[i].word);
        ...
        if(received_result != test_array[i].expected_result){
            printf("[%zd] Failed - Dictionary or words is NULL\n", i);
            }
        else {
            printf("[%zd] Passed\n", i);
        }
